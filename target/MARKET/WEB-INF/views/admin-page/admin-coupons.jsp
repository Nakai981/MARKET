    ...
    <div class="row">
        ...
        <div class="col-7 col-m-12 col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h3>
                        Thêm danh sách sản phẩm cho phiếu # ${idCoupon} ${imp.getId()}
                    </h3>
                </div>
                ...
                <div class="card-content">
                    <table>
                                    ...
                                    <form method="get" action="/admin/coupons/detail/add">
                                        <table>
                                            <tr>
                                                ...
                                                <th> <label>Nhập nhà cung cấp</label></th>
                                                <td>
                                                    <input type="number"  style="height:30px;width: 150px;outline: none;border: none;border-bottom: 2px solid #323232 " name="txtNCC" onkeyup="showResult1(this.value)" value="${imp.getIdSupplier()}"/>
                                                    <div id="ncc1"></div>
                                                </td>

                                            </tr>
                                            <tr> <th> <label>Đơn giá</label></th>
                                                <th><input type="number" style="height:30px;width: 150px;outline: none;border: none;border-bottom: 2px solid #323232 " name="txtDonGia"size="30" value="${imp.getPrice()}"></th>
                                                <th> <label>Số lượng</label></th>
                                                <th><input type="number" style="height:30px;width: 150px;outline: none;border: none;border-bottom: 2px solid #323232 " name="txtSL" size="30" value="${imp.getCount()}"></th>
                                            </tr>
                                            <tr>
                                                <td colspan="2"><button style="height: 30px;cursor:pointer;width: 100%;color: white;border: none; background: #bd4147" type="submit" >${func}</button></td>
                        ...
                        <c:forEach var="i" items="${importCouponsDetail}">
                            <tr>
                                <td> <c:out value = "${i.getNameSupplier()}"/></td>
                                <td> <c:out value = "${i.getNameShipment()}"/></td>
                                <td> <c:out value = "${i.getCount()}"/></td>
                                <td> <c:out value = "${i.getPrice()}"/> VND<p></td>
                                <td><a style="color: #403866" href="/admin/coupons/detail/update1?txtMaDetail=${i.getId()}" ><i class="fas fa-pen-alt"> </i></a></td>
                                <td><a style="color: #bd4147"href="#" onclick="anime(${i.getId()})"><i class="fas fa-trash-alt"></i></a></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
...
<script type="text/javascript">
    ...
    function anime(str){
        if (confirm("Bạn có muốn xóa không")){
            window.location="/admin/coupons/detail/del?id="+str;
        }else{
