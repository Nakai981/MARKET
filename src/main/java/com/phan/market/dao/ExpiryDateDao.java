...
public class ExpiryDateDao {
    ...
    public List<ExpiryDate> selectIteamBySearch(String index){
        List<ExpiryDate> expiryDateList;
        String query = "SELECT LO_HANG.MA_LO,MAT_HANG.TEN_MH FROM LO_HANG INNER JOIN MAT_HANG ON MAT_HANG.MA_MH = LO_HANG.MA_MH WHERE MAT_HANG.ENABLE=1 AND LO_HANG.MA_LO='"+index+"'";
    ...
    public List<ExpiryDate> selectShipment(int index){
        ...
        if (index == -1) {
             query = "SELECT LO_HANG.MA_LO,MAT_HANG.TEN_MH,LO_HANG.SL,LO_HANG.GIA_BAN,LO_HANG.HSD,MAT_HANG.MA_MH FROM LO_HANG\n" +
                    "INNER JOIN MAT_HANG ON LO_HANG.MA_MH=MAT_HANG.MA_MH WHERE  LO_HANG.ENABLE=1 AND MAT_HANG.ENABLE=1  ORDER BY SL DESC";
        }else{
            query = "SELECT LO_HANG.MA_LO,MAT_HANG.TEN_MH,LO_HANG.SL,LO_HANG.GIA_BAN,LO_HANG.HSD,MAT_HANG.MA_MH FROM LO_HANG\n" +
                    "INNER JOIN MAT_HANG ON LO_HANG.MA_MH=MAT_HANG.MA_MH WHERE LO_HANG.MA_MH='"+index+"' AND LO_HANG.ENABLE=1 AND MAT_HANG.ENABLE=1 ORDER BY SL DESC";
        }
    ...
    public int updateShipment(final Shipment shipment, String id){
        String query = "UPDATE LO_HANG SET MA_LO='"+shipment.getId()+"',HSD='"+shipment.getExpiryDate()+"',GIA_BAN='"+shipment.getPrice()+"' WHERE MA_LO='"+id+"'";
        return this.jdbcTemplate.update(query);
    }
